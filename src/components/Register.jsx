...
const Register = () => {
  ...
  const shapes = [
    { size: 60, delay: 0, duration: 20, color: '#2DD4BF' },  // Teal
    { size: 120, delay: 5, duration: 25, color: '#0EA5E9' }, // Sky Blue
    { size: 80, delay: 10, duration: 22, color: '#8B5CF6' }, // Purple
    { size: 100, delay: 15, duration: 28, color: '#0D9488' }, // Dark Teal
  ];
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col lg:flex-row relative overflow-hidden">
      ...
      <div className={`lg:w-1/2 relative overflow-hidden hidden lg:block transition-transform duration-1000 ${mounted ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-teal-500/10 via-sky-500/10 to-purple-500/10 animate-pulse"></div>
        </div>
        <div className="relative h-full flex flex-col justify-center px-12 py-24">
          <div className={`mb-12 transition-all duration-1000 delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <h1 className="text-4xl font-bold bg-gradient-to-r from-teal-400 via-sky-400 to-purple-400 text-transparent bg-clip-text mb-4 animate-gradient">
          ...
          <div className="space-y-8">
            {['Enterprise-Grade Security', 'Real-time Analytics', '24/7 Monitoring'].map((feature, index) => (
              ...
              >
                <div className={`p-2 rounded-lg transform transition-transform hover:scale-110 ${index === 0 ? 'bg-teal-500/10' :
                  index === 1 ? 'bg-sky-500/10' : 'bg-purple-500/10'
                  }`}>
                  <svg className={`w-6 h-6 ${index === 0 ? 'text-teal-400' :
                    index === 1 ? 'text-sky-400' : 'text-purple-400'
                ...
                <div className="group cursor-default">
                  <h3 className="text-slate-100 font-semibold mb-1 group-hover:text-sky-400 transition-colors">{feature}</h3>
      ...
      <div className={`lg:w-1/2 flex items-center justify-center p-8 transition-all duration-1000 ${mounted ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full'}`}>
        <div className="w-full max-w-md relative">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-sky-500/10 to-purple-500/10 rounded-2xl backdrop-blur-xl transform rotate-2 scale-105"></div>

          <div className="relative bg-slate-800/50 p-8 rounded-2xl backdrop-blur-sm border border-slate-700/50 shadow-xl">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold mb-2 animate-gradient bg-gradient-to-r from-teal-400 via-sky-400 to-purple-400 text-transparent bg-clip-text">
                Create Account
              </h2>
              <p className="text-slate-400">Start securing your premises today</p>
            </div>

            {error && (
              <div className="mb-6 p-4 rounded-lg bg-rose-900/30 border border-rose-800/50 text-rose-300 animate-shake">
                {error}
              </div>
            )}

            {success && (
              <div className="mb-6 p-4 rounded-lg bg-teal-900/30 border border-teal-800/50 text-teal-300 animate-success">
                {success}
              </div>
            )}
            ...
            <form onSubmit={handleSignup} className="space-y-6">
              {[
                ...
                >
                  ...
                  <input
                    type={field.type}
                    className="w-full px-4 py-3 rounded-lg bg-slate-800/50 border border-slate-600 text-white placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 transition-all hover:bg-slate-800/70"
              ...
              <button
                type="submit"
                className={`w-full py-4 px-6 rounded-lg bg-gradient-to-r from-teal-500 via-sky-500 to-purple-500 text-white font-medium text-lg transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl disabled:opacity-50 disabled:hover:scale-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
            ...
            <div className={`mt-8 text-center transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
              ...
              <p className="text-slate-400">
                ...
                <Link to="/login" className="text-sky-400 hover:text-sky-300 font-medium transition-colors hover:underline">
                  Sign In
                </Link>
              </p>
            </div>
